...
/**
 * Clase de negocio para gestionar el inventario de vehículos
 * Utiliza HashMap para búsquedas eficientes por ID (O(1))
 */
public class InventarioVehiculos {
    
    // HashMap para almacenamiento eficiente con clave única (identificador)
    ...
    public void agregarVehiculo(Vehiculo vehiculo) throws VehiculoInvalidoException, InventarioException {
        if (vehiculo == null) {
            throw new InventarioException("El vehículo no puede ser nulo");
        }
        
        vehiculo.validar();
        
        if (inventario.containsKey(vehiculo.getIdentificador())) {
            throw new InventarioException("Ya existe un vehículo con la patente: " 
    ...
    public boolean eliminarVehiculo(String identificador) throws InventarioException {
        if (identificador == null || identificador.trim().isEmpty()) {
            throw new InventarioException("El identificador no puede estar vacío");
        }
    ...
    public int cantidadUsados() {
        return obtenerVehiculosUsados().size();
    }
    ...
    public List<Vehiculo> obtenerVehiculosQueNecesitanMantenimiento() {
        ...
        for (Vehiculo v : inventario.values()) {
            if (v instanceof IMantenible) {
                ...
                if (mantenible.necesitaMantenimiento()) {
                    necesitanMantenimiento.add(v);
                }
            }
        }
    ...
    public int getCantidadVehiculos() {
        Collection<Vehiculo> todos = obtenerTodosVehiculos(); // o el método que exponga tus vehículos
    ...
    public int getCantidadUsados() {
        ...
        return (int) todos.stream()
                .filter(v -> v.getCondicion() == CondicionVehiculo.USADO)
                .count();
    }
    ...
    public int getCantidadNuevos() {
        ...
        if (todos == null) return 0;
        return (int) todos.stream()
                .filter(v -> v.getCondicion() == CondicionVehiculo.NUEVO)
                .count();
    }
}